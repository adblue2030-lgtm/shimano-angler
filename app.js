...
function renderResults(fromNo, toNo, afterMin, walkFromMin, walkToMin){
  ...
  if(all.length===0){
    // 時刻の制約を外して「本日その区間に便があるか」を調べ、最終便の情報を案内する
    const anyTime = [...searchDirect(fromNo, toNo, 0), ...searchTransfer(fromNo, toNo, 0)];
    let msg = `この時間以降、条件に合う便が見つかりませんでした。`;
    if(anyTime.length>0){
      const last = anyTime.sort((a,b)=>a.legs[0].board-b.legs[0].board).pop();
      msg += `<br>本日この区間の最終は <b style="color:var(--text)">${minToStr(last.legs[0].board)} ${last.legs[0].fromName}発</b> でした。時刻を早めてお試しください。`;
    }else{
      msg += `<br>この2停留所を直接・乗り継ぎでつなぐ便が時刻表に見つかりませんでした。停留所名をご確認ください。`;
    }
    box.innerHTML = `<div class="panel"><div class="empty"><span class="em">🚌</span>${msg}</div></div>`;
    return;
  }
